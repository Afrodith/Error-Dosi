...
EXTERN_C LIBMWCODERPROFILE_API boolean_T codeInstrInit(
        void ** const ppDispatcher,
        const char* const MATInfoPath,
        const size_t memUnitSize);

/* Parse the message received from the target */
#define X(a) EXTERN_C LIBMWCODERPROFILE_API size_t codeInstrProcessMsg##a( \
        void * const ppDispatcher,                                      \
        void * const data,                                              \
        const size_t length,                                            \
...
EXTERN_C LIBMWCODERPROFILE_API uint32_T codeInstrFinish(
    void * const ppDispatcher,
    mxArray** args[],
    char** cbs[]);

/* Retrieve start callbacks */
EXTERN_C LIBMWCODERPROFILE_API boolean_T codeInstrStartCallbacks(
    void * const pDispatcher,
    int noCallbacks,
    char* cbs[]);

/* Destroy dispatcher */
EXTERN_C LIBMWCODERPROFILE_API void codeInstrDestroy(
